...
// Добавление меток кофеен на карту
function addCoffeePlaces(){
  coffeeData.forEach(coffee=>{
    ...
    const placemark = new ymaps.Placemark(coffee.coordinates,{
      ...
      iconImageHref:'https://github.com/maksdecept97-crypto/coffeemap/raw/main/point.png',
      iconImageSize:[18,28],
    ...
    // Открытие балуна при клике на элемент списка
...
function initEventHandlers(){
  ...
  map.events.add('click',()=>{ coffeePlaces.forEach(cp=>cp.placemark.balloon.close()); });

  // Закрытие списка при клике вне панели
  document.addEventListener('click',e=>{
    const sidebar = document.querySelector('.sidebar');
    const btn = document.querySelector('.show-sidebar-btn');
